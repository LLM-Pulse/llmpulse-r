  ...
  public = list(
    ...
    ListSentimentCategoriesWithHttpInfo = function(project_id, output = NULL, ...) {
      ...
      if (!missing(`project_id`) && is.null(`project_id`)) {
        stop("Invalid value for `project_id` when calling SentimentsApi$ListSentimentCategories, `project_id` is not nullable")
      }

      if (!missing(`output`) && is.null(`output`)) {
        stop("Invalid value for `output` when calling SentimentsApi$ListSentimentCategories, `output` is not nullable")
      }

      query_params[["project_id"]] <- `project_id`

      if (!is.null(`output`) && !(`output` %in% c("flat", "csv"))) {
        stop("Invalid value for output when calling SentimentsApi$ListSentimentCategories. Must be [flat, csv].")
      }
    ...
    ListSentimentRecordsWithHttpInfo = function(project_id, competitor_id = NULL, brand_only = NULL, analysis = NULL, model = NULL, collection_id = NULL, country_code = NULL, language_code = NULL, from = NULL, to = NULL, page = 1, per_page = 20, ...) {
      ...
      if (!is.null(`model`) && !(`model` %in% c("chatgpt", "perplexity", "gemini", "ai_overview", "ai_mode", "copilot", "claude", "grok", "deepseek", "meta_ai", "amazon_rufus", "naver_ai", "baidu_ai"))) {
        stop("Invalid value for model when calling SentimentsApi$ListSentimentRecords. Must be [chatgpt, perplexity, gemini, ai_overview, ai_mode, copilot, claude, grok, deepseek, meta_ai, amazon_rufus, naver_ai, baidu_ai].")
      }
